...
export default function GlobalWarming() {
  const cards = [
    { key: "sustainable", title: "Sustainable Path", tag: "Preferred", tone: "from-emerald-400/30 to-emerald-300/10", accent: "bg-emerald-400/60", stat: "< 1.5°C", blurb: "Design + monitoring + smart materials. Keep assets efficient, resilient and measurable." },
    { key: "critical", title: "Critical Path", tag: "Risk rising", tone: "from-amber-400/30 to-amber-300/10", accent: "bg-amber-400/60", stat: "+ 2.0–2.5°C", blurb: "Higher loads, fatigue and deflections. Real-time data becomes essential for decisions." },
    { key: "failure", title: "Failure Path", tag: "Unacceptable", tone: "from-rose-500/30 to-rose-400/10", accent: "bg-rose-500/60", stat: "> 3.0°C", blurb: "Cascading damage and costly retrofits. Autonomous SHM is survival tech—avoid this future." },
  ];
  ...
  const VIDEO_SRC = "https://res.cloudinary.com/di4esyfmv/video/upload/earth-loop_wd7hay.mp4";
  ...
  return (
    <section className="relative min-h-[calc(100vh-72px)] w-full overflow-hidden bg-black text-white">
      ...
      <div className="relative z-10 mx-auto flex max-w-7xl flex-col gap-10 px-6 py-16 md:py-24">
        ...
        >
          <h1 className="text-3xl font-extrabold tracking-tight md:text-5xl">Global Warming</h1>
          <p className="mx-auto mt-3 max-w-2xl text-sm text-white/70 md:text-base">
            BioStrucX aligns structural design, smart monitoring and sustainable materials— a Space-grade approach for an overheated planet.
          </p>

          {/* === BOTONES CORREGIDOS (React Router) === */}
          <div className="mt-6 flex items-center justify-center gap-3">
            <Link
              to="/launchpad"           // ⬅️ debe existir una <Route path="/launchpad" ... />
            ...
            <Link
              to="/dashboard/jeimie"    // ⬅️ debe existir una <Route path="/dashboard/jeimie" ... />
          ...
          {cards.map((c, idx) => (
            ...
            >
              ...
              <div className="flex items-center justify-between">
                ...
                <span className={`rounded-full ${c.accent} px-2.5 py-1 text-xs font-semibold text-black/90`}>{c.tag}</span>
              </div>
              <div className="mt-10 text-6xl font-extrabold tracking-tight md:text-7xl">{c.stat}</div>
        ...
        <p className="mx-auto max-w-3xl text-center text-sm font-medium bg-gradient-to-r from-emerald-400 via-cyan-400 to-blue-500 bg-clip-text text-transparent">
          Through advanced sensors and integrated biomaterials, BioStrucX Live monitors how structures adapt over time, preventing unnecessary
          demolitions and reducing carbon emissions, while integrating renewable energy systems like solar-powered sensors for sustainable projects.
        </p>
      </div>
    </section>
  );
